...
public class ToastUtil {
    ...
    public static void showToast(String text) {
        if (mToast == null) {
            mToast = ToastCompat.makeText(PushApplication.getInstance(), text, Toast.LENGTH_SHORT);
        } else {
            mToast.cancel();
            mToast = ToastCompat.makeText(PushApplication.getInstance(), text, Toast.LENGTH_SHORT);
        }
    ...
    public static void showToastLong(String text) {
        if (mToast == null) {
            mToast = ToastCompat.makeText(PushApplication.getInstance(), text, Toast.LENGTH_LONG);
        } else {
            mToast.cancel();
            mToast = ToastCompat.makeText(PushApplication.getInstance(), text, Toast.LENGTH_LONG);
        }
